...
/**
 * Escriu un caràcter (tile) al VRAM en una sola trucada.
 *
 * Aquesta funció envia simultàniament les dades de patró (“pat”) i
 * les dades de color (“col”) al processador de vídeo (VDP), situant-les
 * correctament en el bloc corresponent de VRAM segons el grup i l’índex.
 *
 * @param group  Banc de blocs de VRAM (0–3): determina l’àrea vertical
 *               (per exemple, top, middle, bottom) on es carregarà el tile.
 * @param index  Índex dins del bloc (0–255) que identifica la posició
 *               del tile dins de la taula de patrons i de la taula de colors.
 * @param pat    Punter a 8 bytes de dades de patró (8×8 píxels), un byte
 *               per cada línia de píxels del tile.
 * @param col    Punter a 8 bytes de dades de color (atributs de color),
 *               un byte per cada línia, on el nibble alt defineix el
 *               color de primer pla i el nibble baix el color de fons.
 */
void upload_tile_to_vram(uint8_t group, uint8_t index,
                         const uint8_t pat[8], const uint8_t col[8])
{
    uint16_t off = ((uint16_t)group << 11)   /* offset per banc */
                  | ((uint16_t)index << 3);  /* offset per index */

    /* Carrega el patró al bloc de patrons */
    vdp_set_address(VRAM_PATTERN_BASE + off);
    vdp_write_bytes(pat, 8);

    /* Carrega els atributs de color al bloc de colors */
